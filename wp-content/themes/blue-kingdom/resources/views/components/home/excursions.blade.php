...
>
@if(get_field('home_excursions_blocks'))
  ...
  {!! get_field('home_excursions_section_title') !!}
  </h2>
  @endif
  ...
  >
    ...
    <!-- Card 1 -->
     @while(has_sub_field('home_excursions_blocks'))
    <article class="shadow-sm px-4 min-w-60 w-[336px]">
      <div class="w-full">
        ...
        >
          <img
            src="{{get_sub_field('home_excursions_block_image')}}"
            alt="Whale and Wildlife Adventure Zodiac Boat"
            class="object-cover absolute inset-0 size-full"
          />
          @if(get_sub_field('home_excursions_want_to_show_ribbon_text') == 1)
          <p
            class="relative px-5 py-2 max-w-full bg-[#49BCFF] rounded-none w-[168px]"
          >
            {{get_sub_field('home_excursions_ribbon_text')}} 
          </p>
          @endif
        </div>
        <div
        ...
        >
          ...
          {!!get_sub_field('home_excursions_block_title')!!} 
          </h2>
          
          <div class="mt-5 w-full text-zinc-800">
            @if(get_sub_field('home_excursions_inquiry_hour') || get_sub_field('home_excursions_inquiry_age'))
            <div class="flex gap-2.5 items-start w-full">
            @if(get_sub_field('home_excursions_inquiry_hour'))
              <div class="flex flex-1 shrink gap-2 justify-center basis-0">
                <img
                  src="{{ site_url() }}/wp-content/uploads/2025/04/Durationicon.svg"
                  alt="Duration icon"
                  class="object-contain shrink-0 my-auto w-5 aspect-square"
                />
                <p class="flex-1 shrink basis-0">{{get_sub_field('home_excursions_inquiry_hour')}}</p>
              </div>
              @endif
              @if(get_sub_field('home_excursions_inquiry_age'))
              <div class="flex flex-1 shrink gap-2 justify-center basis-0">
                <img
                 src="{{ site_url() }}/wp-content/uploads/2025/04/Ageicon.svg"
                  alt="Age icon"
                  class="object-contain shrink-0 my-auto w-5 aspect-square"
                />
                <p class="flex-1 shrink basis-0">{{get_sub_field('home_excursions_inquiry_age')}}</p>
              </div>
              @endif
            </div>
            @endif
            @if(get_sub_field('home_excursions_inquiry_adults'))
            <div class="flex gap-2 justify-center mt-3.5 w-full">
              <img
               src="{{ site_url() }}/wp-content/uploads/2025/04/Priceicon.svg"
                alt="Price icon"
                class="object-contain shrink-0 my-auto w-5 aspect-square"
              />
              <p class="flex-1 shrink basis-0">{{get_sub_field('home_excursions_inquiry_adults')}}</p>
            </div>
            @endif
            @if(get_sub_field('home_excursions_inquiry_kids'))
            <div class="flex gap-2 justify-center mt-3.5 w-full">
              <img
               src="{{ site_url() }}/wp-content/uploads/2025/04/Priceicon.svg"
                alt="Price icon"
                class="object-contain shrink-0 my-auto w-5 aspect-square"
              />
              <p class="flex-1 shrink basis-0">{{get_sub_field('home_excursions_inquiry_kids')}}</p>
            </div>
            @endif
          </div>
...
@endwhile wp_reset_query();
  ...
  @if(get_field("home_excursions_want_to_show_bottom_shape") == 1)
  @php $image = get_field('home_excursions_shape_image'); @endphp
  @if($image)
    <img src="{{ $image['url'] }}" alt="{{ $image['alt'] ?? 'Decorative wave' }}" class="object-contain absolute right-0 -bottom-px z-0 fill-white w-full max-md:max-w-full" />
  @endif
@endif
</section>
