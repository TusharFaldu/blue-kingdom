<section class="self-stretch px-0 md:py-24 py-10 text-center bg-white">
  ...
  @php $image = get_field('home_map_image') @endphp
@if($image)
  <img src="{{ $image['url'] }}" alt="{{ $image['alt'] ?? 'Background scenic view' }}" class="object-cover absolute inset-0 size-full"/>
@endif
    ...
    <article class="flex flex-col flex-1 shrink justify-center p-11 uppercase rounded shadow-sm basis-0 min-w-60 max-md:px-5 max-md:max-w-full">
      <h2 class="font-heading flex flex-col justify-center w-full text-6xl text-[#142C58] max-md:max-w-full max-md:text-4xl">
        {!! get_field('home_map_section_title') !!}
      </h2>

      <a href="{{get_field('home_map_button_link')}}" class="flex gap-2.5 justify-center items-center self-center px-7 py-3 mt-5 text-base font-semibold text-white rounded bg-[#142C58] max-md:px-5">
        <span class="self-stretch my-auto">
          {{get_field('home_map_direction_button_caption')}}
        </span>
